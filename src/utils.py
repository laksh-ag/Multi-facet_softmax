...
class SeqDataset(torch.utils.data.Dataset):
    def __init__(self, feature,label, bptt, device):
    ...
    def __len__(self):
        return int( self.feature.size(0) /self.seq_len )
    
    def __getitem__(self, idx):
        feature = self.feature[idx*self.seq_len:(idx+1)*self.seq_len].to(dtype = torch.long, device = self.output_device)
        label = torch.cat((torch.zeros(1), self.label[(idx)*self.seq_len:(idx+1)*self.seq_len-1])).to(dtype = torch.long, device = self.output_device)
...
def create_data_loader_split(f_in, bsz, bptt,  device, split_num, dataset_class):
    ...
    # print(type(f_in))
    # w_ind_gpt2_tensor = torch.cat(w_ind_gpt2_tensor)
    feature = w_ind_gpt2_tensor[0]
    labels = w_ind_gpt2_tensor[1]
    training_size = w_ind_gpt2_tensor[0].size(0)
    ...
    for i in range(split_num):
        ...
        dataset_arr.append(  dataset_class(w_ind_gpt2_tensor[0][start:end],w_ind_gpt2_tensor[1][start:end], bptt, device ) ) #assume that the dataset are randomly shuffled beforehand
        # dataset_arr.append(  dataset_class(feature[start:end], bptt, device ) )
        # dataset_labels_arr.append(  dataset_class(labels[start:end], bptt, device ) )
    use_cuda = False
    if device.type == 'cuda':
        use_cuda = True
...
def create_data_loader(f_in, bsz, bptt, device, dataset_class, want_to_shuffle = True):
    w_ind_gpt2_tensor = torch.load(f_in, map_location='cpu')
    dataset = dataset_class(w_ind_gpt2_tensor[0],w_ind_gpt2_tensor[1], bptt, device)
    use_cuda = False
    if device.type == 'cuda':
        use_cuda = True
...
def load_corpus(data_path, train_bsz, eval_bsz, bptt, device, tensor_folder = "tensors", split_num = 1, skip_training = False, want_to_shuffle_val = False, load_testing = False):
    ...
    with open(val_org_corpus_name,'rb') as f_in:
        dataloader_val = create_data_loader(f_in, eval_bsz, bptt, device, dataset_class, want_to_shuffle = want_to_shuffle_val)
