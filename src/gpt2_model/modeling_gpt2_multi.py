...
class GPT2MoSLMHeadModel(GPT2PreTrainedModel):
    ...
    def forward(
        ...
        if self.n_facet_MLP > 0:
            stacked_hidden_emb_raw_arr = torch.cat(hidden_emb_arr, dim=-1)
            hidden_emb_MLP = self.MLP_linear(stacked_hidden_emb_raw_arr)
            #hidden_emb_MLP = self.MLP_linear_l2(gelu(hidden_emb_MLP))
            stacked_hidden_emb_arr = torch.cat([hidden_emb_arr[0], gelu(hidden_emb_MLP)], dim=-1)
            #hidden_emb_MLP_2 = self.MLP_linear_l2(gelu(hidden_emb_MLP))
            #stacked_hidden_emb_arr = torch.cat([hidden_emb_arr[0], gelu(hidden_emb_MLP)+gelu(hidden_emb_MLP_2)], dim=-1)
        else:
            if self.individual_hidden_states:
                stacked_hidden_emb_arr = torch.cat(hidden_emb_arr, dim=-1)
            else:
        ...
        if self.masking_ratio > 0:
            ...
            projected_emb =  self.weight_context(stacked_hidden_emb_arr) * self.get_facet_emb(stacked_hidden_emb_arr, 0)
            projected_emb_arr.append(projected_emb)
            ...
            for j in range(bsz):
                logit_hidden_context = F.linear( projected_emb_arr[0][j,:,:], self.lm_head.weight[input_ids[j,:],:], None)
                logit_hidden_context_before = torch.gather(logit_hidden_context, dim=1, index=self.only_before_index)
                temp = facet_lm_logits_arr[0][j, :, input_ids[j,0]]
                ...
                facet_lm_logits_arr[0][j, :, input_ids[j,0]] = temp + logit_hidden_context[:,0]
            ...
            projected_emb = self.get_facet_emb(stacked_hidden_emb_arr, 0)
            projected_emb_arr.append(projected_emb)
            logit_arr = []
            for j in range(bsz):
                logit_hidden_context = F.linear(projected_emb[j,:,:], self.lm_head.weight[input_ids[j,:],:], None)
                logit_full = torch.zeros( (seq_len, self.vocab_size), device=all_hidden_states[-1].device )
                logit_full[:] = self.other_word_logit[0]
                #logit_full[:] = -1e10
                ...
                logit_hidden_context_before = torch.gather(logit_hidden_context, dim=1, index=self.only_before_index)
                logit_full.scatter_(dim=1, index= input_ids[j,self.only_before_index],src=logit_hidden_context_before)
                
                logit_full[:,input_ids[j,0]] = logit_hidden_context[:,0]
                #logit_full[:,input_ids[j,0]] = -1e10
                logit_arr.append(logit_full)
            logit_full_all = torch.stack(logit_arr,dim=0)

            facet_lm_logits_arr.append(logit_full_all)
            ...
            bsz, seq_len, hidden_size = all_hidden_states[-1].size()
            logit_all = torch.empty( (bsz, seq_len, self.vocab_size) , device=all_hidden_states[-1].device )
            n_facet_not_last = self.n_facet - (n_facet_effective-1)
        ...
        with torch.no_grad():
            ...
            if self.vis_seq_loss or self.vis_simple:
                emb_div_arr_vis = emb_div_arr.view(self.num_vis_bin,-1).mean(dim=-1)
        ...
        if labels is not None:
            ...
            if self.vis_seq_loss or self.vis_simple:
                with torch.no_grad():
                    gt_prob = torch.gather(shift_prediction_prob,  dim=-1 , index = shift_labels.unsqueeze(dim=-1))
                    gt_rank = (gt_prob.expand_as(shift_prediction_prob) <= shift_prediction_prob).type(torch.long).sum(dim = -1)
                    MRR_raw = (1 / gt_rank.type(torch.float))
                    MRR_seq = MRR_raw.mean(dim=0)
                    MRR_seq_vis = MRR_seq.view(self.num_vis_bin_loss,-1).mean(dim=-1)
                    MRR_raw = MRR_raw.view(-1)
                    
                    bsz, seq_len, vocab_size = shift_prediction_prob.size()
                    loss_seq = loss_raw.view(bsz, seq_len).mean(dim=0)
                    loss_seq_vis = loss_seq.view(self.num_vis_bin_loss,-1).mean(dim=-1)
